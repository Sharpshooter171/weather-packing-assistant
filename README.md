...
The project includes a Next.js frontend, an Express/TypeScript backend, Open-Meteo weather and geocoding APIs, OpenStreetMap/Nominatim reverse geocoding for browser current-location display names, SQLite persistence through Prisma, CRUD routes, CSV/JSON export routes, browser current-location support, and a deterministic weather-to-packing recommendation engine.
...
- Reverse geocoding for current-location display names
...
6. For current-location requests, backend reverse-geocodes coordinates into a city/country display name when possible.
7. Backend retrieves weather data from the forecast API.
8. Backend normalizes weather data.
9. Backend detects weather scenarios such as rain, snow, cold, high UV, wind, storms, or mixed conditions.
10. Backend generates travel insights and packing checklist using deterministic rules.
11. Backend stores the request and retrieved weather data in SQLite.
12. Frontend displays current weather, forecast, interpretation, and checklist.
13. Saved requests can be read, updated, deleted, or exported through the API.
```
...
6. Click `Use my current location`, allow browser permission, and show the coordinate-based result with city/country when reverse geocoding is available.
...
- Reverse geocoding is used only to improve the display name for current-location requests; weather data still comes from the weather provider using coordinates.
